...
每条新结果会记录 `评测状态`、`运行ID`、代码提交、工作区状态和当前索引
generation（可获取时）。统计输出会分别展示总题目数、有效题目数、失败题目数、
评测成功率和失败分布。

Agent、Judge 或评测调度故障仍会保留为结果行，便于断点续传和排障，但不会再
作为 0 分混入有效得分。Judge 正常评分得到的真实 0 分仍参与统计。建议比较不同
检索方案时同时报告“有效题目得分率”和“评测成功率”，并确保运行使用同一代码
提交、索引 generation、模型和题目集。

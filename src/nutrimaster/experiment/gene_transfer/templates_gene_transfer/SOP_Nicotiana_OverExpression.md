# 烟草 GoldenBraid2.0表达载体构建及遗传转化完整实验流程 SOP

**物种：** Nicotiana tabacum（普通烟草，异源四倍体，2n=48）/ Nicotiana benthamiana（本氏烟，二倍体，2n=38）  
**系统：** GoldenBraid2.0  
**转化策略：** 叶盘农杆菌介导稳定转化（Agrobacterium-mediated leaf disc transformation）  
**验证策略：** 稳定遗传转化（PCR鉴定+qPCR定量）→ T1代纯合筛选  
**文件版本：** v1.0 | 日期：2026-04-06

## 总体流程概览
NCBI / Sol Genomics Network 烟草基因组数据库
│
▼
...
│
▼
Step 3: 烟草稳定遗传转化（A. tumefaciens LBA4404/GV3101，叶盘法）
│
▼
...
**注意：** 烟草不适用毛状根快速验证系统（此处指稳定转化流程）。烟草叶盘转化体系是植物遗传转化的经典模型，操作简便，转化效率高（20-50%）。N. benthamiana 常用于瞬时表达验证，N. tabacum 用于稳定转化。

## Step 1：获取目的基因序列
### 1.1 目标
从NCBI或SGN数据库获取目的基因CDS编码区序列，用于后续基因表达载体构建及稳定遗传转化。

### 1.2 所需信息
- NCBI登录号
- 烟草参考基因组：
    - N. tabacum：TN90（[https://solgenomics.net/organism/Nicotiana_tabacum/genome）](https://solgenomics.net/organism/Nicotiana_tabacum/genome%EF%BC%89)
    - N. benthamiana：Niben101（[https://solgenomics.net/organism/Nicotiana_benthamiana/genome）](https://solgenomics.net/organism/Nicotiana_benthamiana/genome%EF%BC%89)

### 1.3 操作步骤
**（A）获取基因序列**
1.  访问 Sol Genomics Network（SGN）：[https://solgenomics.net/](https://solgenomics.net/"%20t%20"_blank)
    - 或 NCBI：[https://www.ncbi.nlm.nih.gov/](https://www.ncbi.nlm.nih.gov/"%20t%20"_blank)
...
| --- | --- | --- |
| **数据库** | **网址** | **用途** |
| Sol Genomics Network | https://solgenomics.net/ | 烟草基因序列、注释 |
| NCBI | https://www.ncbi.nlm.nih.gov/ | 序列下载 |
| Nicotiana Genomics | http://www.nicotiana-genomics.org/ | 烟草基因组资源 |
...
**注：pUPD2入门载体为氯霉素抗性，pDGB3_α载体为卡那霉素抗性，pDGB3_Ω载体为壮观霉**
**素抗性。所有连接载体通过蓝白斑筛选，挑取白色克隆进行菌落鉴定。**

## Step 3：烟草稳定遗传转化（叶盘农杆菌介导法）
### 3.1 原理
利用根癌农杆菌（A. tumefaciens LBA4404或GV3101）介导的叶盘转化法，是烟草稳定转化的经典方法，转化效率约20-50%。叶盘外植体操作简便，再生能力强，是植物遗传转化的模式系统。

### 3.2 材料准备
- **菌株：** A. tumefaciens LBA4404（或GV3101）（含过表达载体）
- **烟草品种：** N. tabacum cv. Samsun NN、Xanthi、SR1；N. benthamiana（均易转化）
- **外植体：** 无菌苗叶片（萌发后4-6周）
...
| 共培养基（CCM） | MS盐 + 1 mg/L 6-BA + 0.1 mg/L NAA + 200 μM AS + 0.8%琼脂，pH 5.8 | 共培养 |
| 芽诱导/筛选培养基（SIM） | MS盐 + 1 mg/L 6-BA + 0.1 mg/L NAA + 500 mg/L Cefotaxime + 100 mg/L Kanamycin + 0.8%琼脂，pH 5.8 | 芽诱导与筛选 |
| 芽伸长培养基（SEM） | MS盐 + 0.1 mg/L 6-BA + 500 mg/L Cefotaxime + 100 mg/L Kanamycin + 0.8%琼脂，pH 5.8 | 芽伸长 |
| 生根培养基（RM） | 1/2 MS盐 + 0.1 mg/L IBA + 0.8%琼脂，pH 5.8 | 生根  |

**筛选标记：** 卡那霉素（100 mg/L）为烟草转化首选；也可使用潮霉素（25 mg/L）。
...
### 3.5 外植体制备与侵染
**Day -28 至 Day 0 — 无菌苗培养：**
1.  烟草种子表面消毒：
    - 70%乙醇：1 min
    - 10% NaClO（含0.1% Tween-20）：10 min
    - 无菌水洗涤5次
2.  播种至GM培养基，25°C，16 h光照/8 h黑暗培养4-6周
3.  待叶片完全展开（直径 > 2 cm）

**Day 0 — 外植体制备与侵染：**
1.  在超净工作台中取出无菌苗叶片
2.  用无菌打孔器（直径约1 cm）打取叶盘，或用手术刀切成约1 cm × 1 cm的小块
    - 去除叶脉，保留叶肉组织
3.  将叶盘浸入菌液（OD₆₀₀ = 0.05-0.1）中，室温侵染5-10 min，轻柔摇动
4.  用无菌滤纸吸干多余菌液
5.  将叶盘（近轴面朝下）放置于CCM培养基
6.  25°C，黑暗，共培养2天

### 3.6 芽诱导与筛选
**Day 2 — 转移至筛选培养基：**
1.  用含500 mg/L Cefotaxime的无菌水洗涤叶盘3次（去除农杆菌）
2.  用无菌滤纸吸干，转移至SIM培养基（近轴面朝下）
3.  25°C，16 h光照/8 h黑暗培养
4.  每2周继代一次（转移至新鲜SIM培养基）
5.  培养2-3周，观察芽点生长：
    - 阳性叶盘：在卡那霉素压力下仍可诱导绿色芽点
    - 阴性叶盘：芽点黄化、死亡
...
**转化效率参考：** 烟草叶盘法转化效率约20-50%，每次实验建议处理30-50个叶盘。
